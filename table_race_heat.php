<?php
    ...
    */
    ...
    session_start();
    $panel_heading = ucfirst("Enter Race Results");         
    ...
    FLD_INPUT_NOTEMPTY => false, FLD_INPUT_VALIDATION => 'Numeric',
    FLD_DATABASE => 'idRace'),
    'f2' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Heat#', FLD_DISPLAY_SZ => 7,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    FLD_INPUT_SZ => 7, FLD_INPUT_MAXLEN => 10, FLD_INPUT_DFLT => '',
    ...
    if ($_SERVER['REQUEST_METHOD'] != "POST") {
        ...
        if (($mode != 's') && ($mode != 'i') && ($mode != 'u')) {
            ...
            $mode = $_GET[INP_MODE];
            if (($mode != 's') && ($mode != 'i') && ($mode != 'u') && ($mode != 'd') && ($mode != 'v')) {
                dbu_handle_error(ERR_INVALID_REQUEST);
            }
    } else {
        dbu_handle_error(ERR_INVALID_REQUEST);
    ...
    $sql = "SELECT Description, Race_prefix FROM race where idRace = ".$_SESSION['raceToEnterResults'];
    $result = $dbconn->db_get_one_row($sql);
    if ($result) {
        $raceDescription = $result['Description'];
        $racePrefix = $result['Race_prefix']; 
    }
    echo "<h3>Enter results for race: <FONT COLOR=\"ff0000\">*** ".$raceDescription." ***</FONT></h3>";     
    ...
    switch ($mode) {
        case 'v':
            ...
            $_SESSION['raceHeatForVideo'] = $row['f0']; 
            $show_input = false;  
            $show_data = false;
            // run the heat video assignment script
        ...
        case 'u2':
            ...
            // Error if not all places are assigned
            if (($_POST['f7']==0) or($_POST['f8']==0) or ($_POST['f9']==0) or ($_POST['f10']==0)) {
                errorHandler2("Not all places were assigned.  Race heat results not updated - please try again.");
                break;                
            }

            // Error if racer in first place assigned more than once
            if (($_POST['f7']==$_POST['f8']) or ($_POST['f7']==$_POST['f9']) or ($_POST['f7']==$_POST['f10'])) {
                errorHandler2("Racer assigned to multiple places in the same race.  Race heat results not updated - please try again.");
                break;  
            }
            // Error if racer in second place assigned more than once
            if (($_POST['f8']==$_POST['f9']) or ($_POST['f8']==$_POST['f10'])) {
                errorHandler2("Racer assigned to multiple places in the same race.  Race heat results not updated - please try again.");
                break;  
            }
            // Error if racer in third place assigned more than once
            if ($_POST['f9']==$_POST['f10']) {
                errorHandler2("Racer assigned to multiple places in the same race.  Race heat results not updated - please try again.");
                break;  
            }
            ...
            // If this is the last race_heat:
            // 1. set status to "Complete" in Race table      
            // 2. assign 1st, 2nd, 3rd place racers to race table
            $sqlRH = "SELECT * FROM race_heat WHERE ((idRace =".$_POST['f1'].") and (Place_1=0))";     
            $rsltRH = $dbconn->db_get_all_rows($sqlRH);
            if (!$rsltRH) {
                // $sqlR = "UPDATE race SET Race_status = 'Complete' WHERE idRace=".$_POST['f1'];     
                // $rsltR = $dbconn->db_query($sqlR);

                $sqlRR = "SELECT * FROM race_registration WHERE (rrRace =".$_POST['f1'].") ORDER BY rrPointsEarned";     
                $rsltRR = $dbconn->db_get_all_rows($sqlRR);
                $index = 0;
                foreach ($rsltRR as $key => $value)  {
                    $index++;
                    if ($index == 1) $first = $value['rrRacer'];
                    if ($index == 2) $second = $value['rrRacer']; 
                    if ($index == 3) $third = $value['rrRacer'];                 
                }
                $sqlR = "UPDATE race SET Race_status='Complete', idRacer1=".$first.", idRacer2=".$second.", idRacer3=".$third. " WHERE idRace=".$_POST['f1'];
                $rsltR = $dbconn->db_query($sqlR);  


            }

            // If this is the last race_heat, assign top 3 racers to Race table      
            $sqlRH = "SELECT * FROM race_heat WHERE ((idRace =".$_POST['f1'].") and (Place_1=0))";     
            $rsltRH = $dbconn->db_get_all_rows($sqlRH);
            if (!$rsltRH) {
                $sqlR = "UPDATE race SET Race_status = 'Complete' WHERE idRace=".$_POST['f1'];     
            ...
            // Update score for first place racer
            //addToRacerScore($_POST['f1'], $_POST['f7'], 4);
            // Update score for second place racer
            //addToRacerScore($_POST['f1'], $_POST['f8'], 3);
            // Update score for third place racer
            //addToRacerScore($_POST['f1'], $_POST['f9'], 2);
            // Update score for fourth place racer
            //addToRacerScore($_POST['f1'], $_POST['f10'], 1);

            // Recalculate scores for full race
            recomputeRacerScores($_POST['f1']);
        ...
        <form name="InputForm" method="post" enctype="multipart-form-data"
            ...
            <table border="0">
                <?php  // INPUT
                    ...
                    $racerChoice[0] = "";  
                    $racerChoice[$row['f3']] = "Car# ".$racePrefix.$lane1CarNumber."   - ".$lane1RacerName;
                    $racerChoice[$row['f4']] = "Car# ".$racePrefix.$lane2CarNumber."   - ".$lane2RacerName; 
                    $racerChoice[$row['f5']] = "Car# ".$racePrefix.$lane3CarNumber."   - ".$lane3RacerName; 
                    $racerChoice[$row['f6']] = "Car# ".$racePrefix.$lane4CarNumber."   - ".$lane4RacerName; 
                    ...
                    foreach($fielddef as $fkey=>$fld) {
                        ...
                        case 'Racer_lane1': 
                            ...
                            echo "<td>$lane1RacerName</td><td>Car# $racePrefix$lane1CarNumber</td><td><input name = 'f3' type=hidden value=\" $val\" /></td></tr>";
                            break; 
                        ...
                        case 'Racer_lane2': 
                            ...
                            echo "<td>$lane2RacerName</td><td>Car# $racePrefix$lane2CarNumber</td><td><input name = 'f4' type=hidden value=\" $val\" /></td></tr>";
                            break; 
                        case 'Racer_lane3': 
                            // Show racer name for lane 3 (output only)
                            $val = htmlentities($row[$fkey], ENT_QUOTES, $config['encoding']); 
                            echo "<tr><td>Racer - Lane #3:</td>";  
                            echo "<td>$lane3RacerName</td><td>Car# $racePrefix$lane3CarNumber</td><td><input name = 'f5' type=hidden value=\" $val\" /></td></tr>";
                            break; 
                        ...
                        case 'Racer_lane4': 
                            ...
                            echo "<td>$lane4RacerName</td><td>Car# $racePrefix$lane4CarNumber</td><td><input name = 'f6' type=hidden value=\" $val\" /></td></tr>";
                            break; 
    ...
    <?php } else if ($show_data) { ?>
            ...
            <table cellpadding="1" cellspacing="0" border="0" bgcolor="#ababab"><tr><td>
                        <table cellpadding="0" cellspacing="1" border="0" class="datatable">
                            <tr><th style="width: 25px;"></th>
                                <?php  // DATA HEADER
                                    // Initialize counter for rows written
                                    $rowsInTable = 0;
                                    foreach ($fielddef as $fkey=>$fld) {
                                        if ($fld[FLD_DISPLAY]) {
                        ...
                        <?php  // DATA
                            ...
                            $i = 0;
                            foreach($rows as $row) {
                                // Update counter for rows in the table
                                $rowsInTable++;
                                // Get count of heats already run
                                $heatCount = 0;
                                $sqlRH = "SELECT * FROM race_heat WHERE ((idRace =".$row['f1'].") and (Place_1<>0))";     
                                $rsltRH = $dbconn->db_get_all_rows($sqlRH);
                                if ($rsltRH) $heatCount = count($rsltRH);
                                $locateRadio = $heatCount + 1;

                                if ($rowsInTable == $locateRadio) {
                                    $checked = ' checked="checked"';
                                } else {
                                    $checked = '';
                                }
                                ...
                                foreach ($fielddef as $fkey=>$fld) {
                                    ...
                                    switch ($fld[FLD_DATABASE]) {
                                        ...
                                        case 'Number';
                                            $sql = "SELECT * from race_heat_video where RHVidRace_Heat=".$row['f0'];
                                            $result = $dbconn->db_get_one_row($sql);
                                            if ($result) {
                                                $raceVideoFlag = ' v';
                                            } else {
                                            ...
                                            $outputNumber = $row['f2'];
                                            echo "<td>$outputNumber<font size = 1>$raceVideoFlag</font></td>";
                                            break;
        ...
        <table cellpadding="1" cellspacing="0" border="0" bgcolor="#ababab"><tr><td>
                    <table cellpadding="1" cellspacing="0" border="0" bgcolor="#fcfcfc"><tr><td>
                                <?php
                                    if ($rowsInTable > 0) {
                                        echo "<input type=\"button\" value=\"update\" onclick=\"document.forms.ActionForm.action='?mode=u'; document.forms.ActionForm.submit()\" />";
                                        echo "<input type=\"button\" value=\"assign video\" onclick=\"document.forms.ActionForm.action='?mode=v'; document.forms.ActionForm.submit()\" />";
                                    }
                                ?>  
                            </td></tr>
                    </table>
