<?php
    ...
    if ($config['autoRefreshON'] == 'YES') {
        ...
        // - if time to display racer points, go to that display now
        $_SESSION['autoRefreshControl#1'] = $_SESSION['autoRefreshControl#1'] + 1;
        if ($_SESSION['autoRefreshControl#1'] > $config['autoRefreshHeatDisplayCount']) {
            $_SESSION['autoRefreshControl#1'] = 0;
            echo '<meta http-equiv="refresh" content="0; URL=table_race_registration_display.php">';   
        }
    }
    ...
    $panel_heading = ("Race Results: ".$raceDescription."  (".$lastRaceHeat. " of ".$resultHH['Heats']." heats complete)");   
