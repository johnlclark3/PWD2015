<?php
    ...
    */
    ...
    'f5' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'School grade', FLD_DISPLAY_SZ => 20,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    FLD_INPUT_SZ => 20, FLD_INPUT_MAXLEN => 20, FLD_INPUT_DFLT => '',
    FLD_INPUT_NOTEMPTY => true, FLD_INPUT_VALIDATION => 'Numeric',
    FLD_DATABASE => 'Den_grade')
    );
    ...
    switch ($mode) {
        ...
        case 'u2':
            ...
            break;
        case 'd':
            // only permit delete if there are no racers in that den
            $sqlR = "SELECT * FROM racer WHERE Den_IdDen =".$_POST['RKEY'];     
            $rsltR = $dbconn->db_get_all_rows($sqlR);
            if ($rsltR) {
                errorHandler2("Den cannot be deleted if there are racers that belong to that den.  Den not deleted.");
                break;   
            }
                ...
                <?php  // INPUT
                    foreach($fielddef as $fkey=>$fld) {
                        ...
                        case 'Den_grade': 
                            ...
                            $val = htmlentities($row[$fkey], ENT_QUOTES, $config['encoding']);
                            // Punch out start of HTML
                            echo "<tr><td>School grade</td>";
                            echo "<td><select name = 'f5'>\n";
                            // Output each individual drop down entry
                            foreach ($schoolGrade as $key => $choice)         {
                                echo "<option value='$key' ";
                                // If the entry is the same as the existing database value
                                // indicate this entry shoudl show (selected)
                                // (used for existing database records)
            ...
            <table cellpadding="1" cellspacing="0" border="0" bgcolor="#ababab"><tr><td>
                        <table cellpadding="0" cellspacing="1" border="0" class="datatable">
                            <tr><th style="width: 25px;"></th>
                                <?php  // DATA HEADER
                                    foreach ($fielddef as $fkey=>$fld)  {
                                        if (($fld[FLD_DISPLAY]) and ($fld[FLD_DATABASE] <> 'Den_grade')) {
                        ...
                        <?php  // DATA
                            ...
                            foreach($rows as $row) {
                                ...
                                foreach ($fielddef as $fkey=>$fld) {
                                    if (($fld[FLD_VISIBLE]) and ($fld[FLD_DATABASE] <> 'Den_grade')) {
