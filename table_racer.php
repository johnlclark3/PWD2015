<?php
    ...
    */
    ...
    FLD_DATABASE => 'idRacer'),
    'f1' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'First name', FLD_DISPLAY_SZ => 25,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    FLD_INPUT_SZ => 20, FLD_INPUT_MAXLEN => 136, FLD_INPUT_DFLT => '',
    FLD_INPUT_NOTEMPTY => true, FLD_INPUT_VALIDATION => '',
    FLD_DATABASE => 'First_name'),
    'f2' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Last name', FLD_DISPLAY_SZ => 25,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    FLD_INPUT_SZ => 20, FLD_INPUT_MAXLEN => 136, FLD_INPUT_DFLT => '',
    FLD_INPUT_NOTEMPTY => true, FLD_INPUT_VALIDATION => '',
    FLD_DATABASE => 'Last_name'),
    'f3' => array(FLD_ID => false, FLD_VISIBLE => false, FLD_DISPLAY => '', FLD_DISPLAY_SZ => 7,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    FLD_INPUT_SZ => 7, FLD_INPUT_MAXLEN => 10, FLD_INPUT_DFLT => '',
    FLD_INPUT_NOTEMPTY => true, FLD_INPUT_VALIDATION => 'Numeric',
    FLD_DATABASE => 'Den_IdDen')
    );
    ...
    switch ($mode) {
        case 's':
            ...
            $rows = dbu_handle_select($fielddef, $scheme, $table, $dbconn, $keys, $start, $config['rows_per_page'], $config['pager_items'], $pager, 'last_name,first_name');
        ...
        case 'd':
            ...
            // only permit delete if the race is not on any race-registration records
            $sqlRR = "SELECT * FROM race_registration WHERE rrRacer =".$_POST['RKEY'];     
            $rsltRR = $dbconn->db_get_all_rows($sqlRR);
            if ($rsltRR) {
                errorHandler2("Racer cannot be deleted if registered for any races.  Racer not deleted.");
                break;   
            }
            ...
            <table border="0">
                <?php  // INPUT
                    foreach($fielddef as $fkey=>$fld) {
                        ...
                        if ($fld[FLD_DATABASE] == 'Den_IdDen') {

                            // Be sure to have dtaabase value ready
                            $val = htmlentities($row[$fkey], ENT_QUOTES, $config['encoding']);

                            // Assemble an array of dens tro show in drop down selection box
                            $sql = "SELECT * FROM den order by Den_grade, Den_number";
                            $result = $dbconn->db_get_all_rows($sql);
                            if (!$result) {
                                echo "falure </br>"; 
                            } else {
                                foreach($result as $key => $value) {
// JClark 2014-12-11 replace next line - was creating error about syntax
//                                    $denArray[$value[IdDen]] = $value[Den_name];
                            ...
                            foreach ($denArray as $key => $choice)         {
            ...
            <table cellpadding="1" cellspacing="0" border="0" bgcolor="#ababab"><tr><td>
                        <table cellpadding="0" cellspacing="1" border="0" class="datatable">
                            <tr><th style="width: 25px;"></th>
                                ...
                                echo "<th style=\"width: 25ex\">Den</th>";  
                            ?>
                        </tr>
                        <?php  // DATA
                            ...
                            foreach($rows as $row) {
                                ...
                                // Get den name
                                $denName = '';
                                $sqlDen = "SELECT * FROM den WHERE idDen =".$row['f3'];     
                                $resultDen = $dbconn->db_get_one_row($sqlDen);
                                if ($resultDen) $denName = $resultDen['Den_name'];
