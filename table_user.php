<?php
    ...
    FLD_DATABASE => 'idUser'),
    'f1' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'User name', FLD_DISPLAY_SZ => 100,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    FLD_INPUT_SZ => 100, FLD_INPUT_MAXLEN => 49, FLD_INPUT_DFLT => '',
    FLD_INPUT_NOTEMPTY => true, FLD_INPUT_VALIDATION => 'AlphaNum',
    FLD_DATABASE => 'username'),
    'f2' => array(FLD_ID => false, FLD_VISIBLE => false, FLD_DISPLAY => '', FLD_DISPLAY_SZ => 100,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'hidden',
    FLD_INPUT_SZ => 100, FLD_INPUT_MAXLEN => 121, FLD_INPUT_DFLT => '',
    FLD_INPUT_NOTEMPTY => true, FLD_INPUT_VALIDATION => '',
    FLD_DATABASE => 'passwd'),
    'f3' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Administrator?', FLD_DISPLAY_SZ => 100,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    FLD_INPUT_SZ => 100, FLD_INPUT_MAXLEN => 4, FLD_INPUT_DFLT => '',
    FLD_INPUT_NOTEMPTY => true, FLD_INPUT_VALIDATION => '',
    FLD_DATABASE => 'admin'),
// JClark 2014-12-11 remove input validation 'Email' - creating errors - and not seeming to work
//  'f4' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Email Address', FLD_DISPLAY_SZ => 100,
//  FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
//  FLD_INPUT_SZ => 100, FLD_INPUT_MAXLEN => 301, FLD_INPUT_DFLT => '',
//  FLD_INPUT_NOTEMPTY => true, FLD_INPUT_VALIDATION => 'EMail',
//  FLD_DATABASE => 'email')
    'f4' => array(FLD_ID => false, FLD_VISIBLE => true, FLD_DISPLAY => 'Email Address', FLD_DISPLAY_SZ => 100,
    FLD_INPUT => true, FLD_INPUT_TYPE => 'text',
    FLD_INPUT_SZ => 100, FLD_INPUT_MAXLEN => 301, FLD_INPUT_DFLT => '',
    ...
    switch ($mode) {
        case 's':
            ...
            $rows = dbu_handle_select($fielddef, $scheme, $table, $dbconn, $keys, $start, $config['rows_per_page'], $config['pager_items'], $pager, 'username');
        ...
        case 'u2':
            ...
            // If there is a value in the new password field, encrypt it!
            if ($_POST['f2n'] <> '****************') {
                $_POST['f2'] = sha1($_POST['f2n']);
            }
                ...
                <?php  // INPUT
                    foreach($fielddef as $fkey=>$fld) {
                        switch ($fld[FLD_DATABASE]) {
                            // If this is the "administrator?" field, show as a "Yes" or "No" drop down box
                            case 'admin': 
                                // Be sure to have dtaabase value ready
                                $val = htmlentities($row[$fkey], ENT_QUOTES, $config['encoding']);

                                // Assemble an array of options for whether user is an admin
                                $adminOptions['N'] = 'Not an administrator';
                                $adminOptions['Y'] = 'Is an administrator';

                                // Punch out start of HTML
                                echo "<tr><td>Administrator?</td>";
                                echo "<td><select name = 'f3'>\n";

                                // Output each individual drop down entry
                                foreach ($adminOptions as $key => $choice)         {
                                    echo "<option value='$key' ";

                                    // If the entry is the same as the existing database value
                                    // indicate this entry shoudl show (selected)
                        ...
                        case 'passwd':  
                            // Be sure to have dtaabase value ready
                            $val = htmlentities($row[$fkey], ENT_QUOTES, $config['encoding']);
                            // Assign a temp value to password to keep editing routines happy till the new value is assigned
                            if ($val = ''); {
                                $val = 'tempvalue';
                            }
                            // Punch out HTML
                            echo "<tr><td>New password</td>";
                            echo "<td><input name = 'f2n' type=\"text\" size=\"16\" value=\"****************\"></td><tr>\n";
                            echo "<tr><td><input name = 'f2' type=\"hidden\" value=\"$val\"></td><tr>\n";
                            break;
                        default:
                            if ($fld[FLD_INPUT]) {
    ...
    <?php } else if ($show_data) { ?>
        ...
        <table cellpadding="1" cellspacing="0" border="0" bgcolor="#ababab"><tr><td>
                    <table cellpadding="1" cellspacing="0" border="0" bgcolor="#fcfcfc"><tr><td>
                                <input type="button" value="new user" onclick="document.forms.ActionForm.action='?mode=i'; document.forms.ActionForm.submit()" />&nbsp;
